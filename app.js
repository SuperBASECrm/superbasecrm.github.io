...
function wireFormHandlers() {
  ...
  const closeTemplateDeleteX = document.getElementById('closeTemplateDeleteX');
  if (closeTemplateDeleteX) {
    closeTemplateDeleteX.addEventListener('click', () => toggleTemplateDeleteModal(false));
  }
  const templateDeleteModal = document.getElementById('templateDeleteModal');
  if (templateDeleteModal) {
    templateDeleteModal.addEventListener('click', (e) => {
      if (e.target === templateDeleteModal) toggleTemplateDeleteModal(false);
    });
  }
...
async function handleCompanySubmit(event) {
  ...
  const { error } = await supabaseClient.from('companies').insert(newCompany);
...
async function handleContactSubmit(event) {
  ...
  if (emailRaw) {
    setButtonLoading(submitButton, true);
    const { data: dupMatches, error: dupError } = await supabaseClient
      .from('contacts')
  ...
  setButtonLoading(submitButton, true);
  const { error } = await supabaseClient.from('contacts').insert(newContact);
  setButtonLoading(submitButton, false);
...
function deleteTemplate() {
  ...
  supabaseClient
    ...
    .then((versionRes) => {
      ...
      return supabaseClient.from('email_templates').delete().eq('id', tplId);
    })
    .then((deleteRes) => {
      if (!deleteRes) return;
...
async function markTaskComplete(taskId) {
  ...
  if (existing && followUpDate) {
    ...
    const { error: createError } = await supabaseClient.from('tasks').insert(clonePayload);
    if (createError) {
...
async function handleContactSequenceEnroll() {
  ...
  const { error } = await supabaseClient.from('contact_sequence_enrollments').insert(payload);
  if (error) {
...
async function updateSequenceTaskDueDate(taskId, newDate) {
  ...
  const { error } = await supabaseClient.from('tasks').update({ due_date: newDate }).eq('id', taskId);
  if (error) {
...
async function handleCompanyDelete() {
  ...
  if (contactIds.length > 0) {
    const { error: taskByContactError } = await supabaseClient
      .from('tasks')
      .delete()
  ...
  const { error: taskByCompanyError } = await supabaseClient
    .from('tasks')
    .delete()
  ...
  const { error: oppError } = await supabaseClient.from('opportunities').delete().eq('company_id', companyId);
  if (oppError) {
  ...
  const { error: contactError } = await supabaseClient.from('contacts').delete().eq('company_id', companyId);
  if (contactError) {
  ...
  const { error: companyError } = await supabaseClient.from('companies').delete().eq('id', companyId);
  setButtonLoading(deleteBtn, false, 'Delete Company');
  if (companyError) {
...
async function handleContactDelete() {
  ...
  const { error: taskError } = await supabaseClient.from('tasks').delete().eq('contact_id', contactId);
  if (taskError) {
  ...
  const { error: contactError } = await supabaseClient.from('contacts').delete().eq('id', contactId);
  setButtonLoading(deleteBtn, false, 'Delete');
  if (contactError) {
...
async function handlePipelineDelete() {
  ...
  const { error: pipeError } = await supabaseClient.from('pipelines').delete().eq('id', pipelineId);
  setButtonLoading(deleteBtn, false, 'Delete');
  if (pipeError) {
...
async function savePipelineEdit() {
  ...
  if (state.editingDeletedStageIds.length > 0) {
    ...
    const { error: oppDeleteError } = await supabaseClient
      .from('opportunities')
      .delete()
  ...
  if (currentPipeline && currentPipeline.name !== newName) {
    const { error: pipeUpdateError } = await supabaseClient
      .from('pipelines')
      .update({ name: newName })
      .eq('id', pipelineId);
    if (pipeUpdateError) {
...
async function handleOpportunityDelete() {
  ...
  const companyWasOpen =
    companyDetailModal && !companyDetailModal.classList.contains('hidden') && state.selectedCompanyId;
  const companyIdToRefresh = companyWasOpen ? state.selectedCompanyId : null;
  const { error } = await supabaseClient.from('opportunities').delete().eq('id', oppId);
  if (error) {
...
async function loadSequencesAndSteps() {
  ...
  if (latestVersionIds.length > 0) {
    const { data: versionStepsData, error: versionStepsError } = await supabaseClient
      .from('sequence_step_versions')
      .select(
...
async function handleSequenceFormSubmit(event) {
  ...
  const submitButton = event.target.querySelector('button[type="submit"]');
  setButtonLoading(submitButton, true);
  const { data, error } = await supabaseClient.from('sequences').insert({ name }).select().single();
  setButtonLoading(submitButton, false);
...
async function ensureTaskVersionResolution(task) {
  ...
  if (!step) {
    ...
    const { data: stepRow, error } = await supabaseClient
      .from('sequence_step_versions')
      .select(
        'id,sequence_version_id,original_step_id,step_order,delay_days,name,step_type,position,template_id,template_subject,template_body'
      )
    ...
    if (error) {
      ...
      if (!task.sequence_version_id && versionId) {
        await supabaseClient.from('tasks').update({ sequence_version_id: versionId }).eq('id', task.id);
        state.tasks = state.tasks.map((t) =>
...
async function saveSequenceEdit() {
  ...
  const sequence = state.sequences.find((s) => String(s.id) === seqKey);
  const { data: newVersion, error: versionInsertError } = await supabaseClient
    .from('sequence_versions')
    .insert({
...
async function handleSequenceDelete() {
  ...
  const { data: versions, error: versionFetchError } = await supabaseClient
    .from('sequence_versions')
    .select('id')
    .eq('sequence_id', seqId);
  if (versionFetchError) {
    ...
    if (versionIds.length > 0) {
      const { error: versionStepsDeleteError } = await supabaseClient
        .from('sequence_step_versions')
        .delete()
  ...
  const { error: seqError } = await supabaseClient.from('sequences').delete().eq('id', seqId);
  setButtonLoading(deleteBtn, false, 'Delete');
  if (seqError) {
